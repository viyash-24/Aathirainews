...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetchCategories(),
      fetchAdminNews({ limit: 1000 }), // Fetch articles to compute counts
    ])
      .then(([catsRes, newsRes]) => {
  ...
  const toggleStatus = async (cat: CategoryData) => {
    try {
      const newStatus = cat.status === "Active" ? "Inactive" : "Active";
      const updated = await updateCategory(cat._id, { status: newStatus });
            ...
            <div className="bg-white rounded-xl p-8 max-w-sm w-full mx-4 shadow-2xl">
              <div className="text-center mb-6">
                ...
                <p className="text-slate-500 text-sm mt-1">
                  This action cannot be undone. All articles under this category will be unassigned.
                </p>
              </div>
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 border-b border-gray-200 pb-4 gap-4">
          <div>
            ...
            <p className="font-[Inter] text-[14px] leading-[20px] text-slate-500">
              {categories.length} categories · {categories.filter((c) => c.status === "Active").length} active
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-12 gap-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6 sticky top-6">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">English Name *</label>
                    <input
                      className={`w-full border ${errors.name ? "border-primary" : "border-gray-200"} p-2.5 focus:ring-2 focus:ring-blue-900 outline-none text-sm`}
                  ...
                  {/* Tamil Name */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Tamil Name (தமிழ்) *</label>
                    <input
                      className={`w-full border ${errors.nameTa ? "border-primary" : "border-gray-200"} p-2.5 focus:ring-2 focus:ring-blue-900 outline-none font-[Mukta_Malar] text-[16px]`}
                      placeholder="எ.கா. அரசியல்"
                      value={form.nameTa}
                      onChange={(e) => setForm({ ...form, nameTa: e.target.value })}
                    />
                    {errors.nameTa && <p className="text-primary text-xs">{errors.nameTa}</p>}
                  </div>

                  {/* Slug */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">URL Slug *</label>
                    <div className="flex items-center border border-gray-200 focus-within:ring-2 focus-within:ring-blue-900">
                      <span className="px-3 py-2.5 bg-slate-50 text-slate-400 text-xs border-r border-gray-200 select-none">/category/</span>
                      <input
                    ...
                    {errors.slug && <p className="text-primary text-xs">{errors.slug}</p>}
                  </div>

                  {/* Color & Status */}
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1">
                      <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Color</label>
                  ...
                  <div className="bg-slate-50 p-3 rounded-lg">
                    <p className="text-[10px] text-slate-400 font-bold uppercase mb-2">Preview</p>
                    <span className="inline-block text-white text-xs font-bold px-3 py-1 uppercase tracking-wider" style={{ backgroundColor: form.color }}>
                      {form.name || "Category Name"}
                    </span>
                    <span className="ml-2 font-[Mukta_Malar] text-sm text-slate-600">{form.nameTa || "வகை பெயர்"}</span>
                  </div>

                  <div className="flex gap-3 pt-2">
                    <button onClick={() => setShowForm(false)} className="flex-1 py-2.5 border border-slate-300 text-slate-600 font-bold hover:bg-slate-50 transition-colors text-sm">
                      Cancel
                    </button>
                    <button onClick={handleSubmit} className="flex-1 py-2.5 bg-blue-900 text-white font-bold hover:bg-blue-800 transition-colors text-sm">
          ...
          <div className={`col-span-12 ${showForm ? "lg:col-span-8" : ""}`}>
            <div className="mb-4 flex gap-3 items-center">
              ...
              <span className="text-sm text-slate-500">{filtered.length} result(s)</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider hidden sm:table-cell">Articles</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center text-slate-400 text-sm">Loading categories...</td>
                    </tr>
                  ) : filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center text-slate-400 text-sm">No categories found.</td>
                    </tr>
                  ) : (
                    filtered.map((cat) => {
                      const count = articleCounts[cat.name] || articleCounts[cat.name.toUpperCase()] || 0;
                      return (
                        <tr key={cat._id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="font-[Mukta_Malar] text-xs text-slate-400">{cat.nameTa}</p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <button
                              ...
                              className={`flex items-center gap-1.5 text-xs font-bold uppercase cursor-pointer ${cat.status === "Active" ? "text-emerald-600" : "text-slate-400"}`}
                            >
                              <span className={`w-2 h-2 rounded-full ${cat.status === "Active" ? "bg-emerald-500" : "bg-slate-300"}`}></span>
                              {cat.status}
                            </button>
                          </td>
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-1">
                              <button onClick={() => handleEdit(cat)} className="p-1.5 text-slate-400 hover:text-blue-900 hover:bg-blue-50 rounded transition-colors" title="Edit">
                                <span className="material-symbols-outlined text-[18px]">edit</span>
                              </button>
                              <button onClick={() => setDeleteConfirmId(cat._id)} className="p-1.5 text-slate-400 hover:text-primary hover:bg-red-50 rounded transition-colors" title="Delete">
